...
public class RabbitProducerImpl<T extends Serializable> implements RabbitProducer<T> {
    ...
    public void convertAndSend(
        ...
        try {
            log.info(
                    "Sending rabbit message, exchange={}, routingKey={}, messageId={}, message={}",
                    exchange,
                    routingKey,
                    messageId,
                    message);
            ...
            log.error(
                    "Error on rabbit producer, routingKey={}, message={}", routingKey, message, e);
            throw new RuntimeException(
                    "Error on rabbit producer with routingKey: "
                            + routingKey
                            + ", message: "
                            + message,
                    e);
